...
namespace ccLight.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                if (Session["skey"] != null && Session["skey"].ToString().ToLower() == Request["securityCode"].ToLower())
                {
                    var user = UserManager.Find(model.Cname, model.Password);
                    if (user != null)
                    {
                        if (user.Stat == 0)
                        {
                            ModelState.AddModelError("", "用户锁定状态，请联系管理员为您激活！");
                        }
                        else
                        {
        ...
        public async Task<ActionResult> Register(AppUser appuser)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var user = new AppUser() { UserName = appuser.UserName, Cname = appuser.Cname, Mobile = appuser.Mobile, Email = appuser.Email };
        ...
        {
            var userInfo = (Session["user"] as AppUser);
        ...
        private async Task SignInAsync(AppUser user, bool isPersistent)
        {
            ...
            UserAuth.getAuths(user, ref auths, ref modules, ref moduleIds);
            //保持用户及权限信息到全局中
            Session["cchp.auth"] = auths;
            Session["mids"] = moduleIds;
